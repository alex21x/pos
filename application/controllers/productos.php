...
class Productos extends CI_Controller
{
    ...
    public function crear()
    {
        ...
        $data['rubros'] = $this->tipo_cliente_rubros_model->select();
   ...
   public function buscar_producto(){
    ...
    public function kardex_fisico_producto(){
        ...
        $this->db->join('empleados as e','e.id = k.k_vendedor','LEFT');
   ...
   public function selectPrecioCodBarra() {
      ...
      //CODIGO DE BARRAS 10-11-2020
   public function selectPrecioCodProducto() {        
        $rowPrecioCodProducto = array();        
        $rowPrecioCodProducto = $this->productos_model->selectPrecioCodProducto($_POST['codProducto']);
                
        $array = array(            
                 'prod_id' => $rowPrecioCodProducto->prod_id,
                 'prod_nombre' => $rowPrecioCodProducto->prod_nombre,
                 'prod_precio_publico' => $rowPrecioCodProducto->prod_precio_publico,
                 'prod_stock' => $rowPrecioCodProducto->prod_stock,
                 'prod_medida'=> $rowPrecioCodProducto->medida_nombre
                );        
        echo json_encode($array);
    }    
    ...
    public function exportarKardex(){
        ...
        $fecha_desde   = $_GET["fechai"];
        $fecha_hasta   = $_GET["fechaf"];

        $almacenText   = $_GET["almacenText"];
        //$productoText  = $_GET["productoText"];

        if ($fecha_desde != '') {
             $this->db->where('DATE(k.k_fecha) >=',(new DateTime($fecha_desde))->format("Y-m-d"));
        }
         if ($fecha_hasta != '') {
             $this->db->where('DATE(k.k_fecha) <=',(new DateTime($fecha_hasta))->format("Y-m-d"));
        }

            $rsKardex = $this->db->from('kardex as k')
                                 ->join('productos as p','p.prod_id = k.k_producto')
                                 ->join('almacenes as a','a.alm_id = k.k_almacen')
                                 ->join('empleados as e','e.id = k.k_vendedor','LEFT')
                                 ->where('k.k_almacen',$almacen)
                                 ->where('k.k_producto',$producto)
                                 ->order_by('k.k_id','ASC')
                                 ->get()
                                 ->result();         
        ...
        $spreadsheet->setActiveSheetIndex(0)                
                ->setCellValue('B1', 'REPORTE KARDEX');

        $spreadsheet->getActiveSheet()
                ->setCellValue('A2', 'ALMACEN')
                ->setCellValue('B2', $almacenText);
        ...
        $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('B6', 'ALMACEN')
                ->setCellValue('C6', 'USUARIO')
                ->setCellValue('D6', 'PRODUCTO')
                ->setCellValue('E6', 'DOCUMENTO')
                ->setCellValue('F6', 'CONCEPTO')
                ->setCellValue('G6', 'ENTRADAS')
                ->setCellValue('H6', 'SALIDAS')
                ->setCellValue('I6', 'EXISTENCIAS');

        $rsTotal_cantidad = 0;              
        $rsTotal_stockValorizado = 0;

        foreach($rsKardex  as $value) {            
                
            $spreadsheet->getActiveSheet()
                        ->setCellValue('A'.$i, $value->k_fecha)
                        ->setCellValue('B'.$i, $value->alm_nombre)
                        ->setCellValue('C'.$i, $value->nombre.' '.$value->apellido_paterno.' '.$value->apellido_materno)
                        ->setCellValue('D'.$i, $value->prod_nombre)
                        ->setCellValue('E'.$i, $value->k_serie)
                        ->setCellValue('F'.$i, $value->k_concepto)
                        ->setCellValue('G'.$i, $value->k_ecantidad)
                        ->setCellValue('H'.$i, $value->k_scantidad)
                        ->setCellValue('I'.$i, $value->k_excantidad);                
