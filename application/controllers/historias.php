...
class Historias extends CI_Controller
{
    ...
    public function decargarPdf($idHistoria){
        ...
        $rsHistoria = $this->db->select('his.his_id his_id,his.his_correlativo his_correlativo,DATE_FORMAT(his.his_fecha, "%d-%m-%Y %h:%i:%s") his_fecha,his.his_ini_peso,his.his_ini_talla,his.his_ini_presion_arterial,his.his_ini_temperatura,his.his_ini_otros,his.his_codigo_cie,his.his_enfermedad_actual,his.his_motivo,his.his_diagnostico,his.his_tratamiento,his.his_recomendacion,his.his_documento_venta,DATE_FORMAT(his.his_fecha_cita, "%d-%m-%Y %h:%i:%s") his_fecha_cita,pac.id paciente_id,pac.razon_social pac_razon_social,prof.prof_nombre prof_nombre,prof.prof_firma prof_firma,esp.esp_descripcion,CONCAT(emp.nombre," ",emp.apellido_paterno) empleado,hie.hie_descripcion estado,tpe.descripcion his_codigoCEI_descripcion, his.his_fin_peso, his.his_fin_talla,his.his_fin_temperatura, his.his_fin_presion_arterial, his.his_fin_otros',FALSE)
                            ->from('historias his')
    ...
    {        
        ...
        $rsHistoria = $this->db->select('his.his_id his_id,his.his_correlativo his_correlativo,DATE_FORMAT(his.his_fecha, "%d-%m-%Y %h:%i:%s") his_fecha,his.his_ini_peso,his.his_ini_talla,his.his_ini_presion_arterial,his.his_ini_temperatura,his.his_ini_otros,his.his_enfermedad_actual,his.his_motivo,his.his_diagnostico,his.his_tratamiento,his.his_recomendacion,his.his_documento_venta,DATE_FORMAT(his.his_fecha_cita, "%d-%m-%Y %h:%i:%s") his_fecha_cita,pac.id paciente_id,pac.razon_social pac_razon_social,prof.prof_nombre prof_nombre,prof.prof_firma prof_firma,esp.esp_descripcion,CONCAT(emp.nombre," ",emp.apellido_paterno) empleado,hie.hie_descripcion estado,his.his_fin_peso, his.his_fin_talla, his.his_fin_temperatura, his.his_fin_presion_arterial, his.his_fin_otros',FALSE)
                            ->from('historias his')
                            ->join('pacientes pac','pac.id = his.his_paciente_id')
                            ->join('profesionales prof','prof.prof_id =  his_profesional_id')
                            ->join('especialidades esp','esp.esp_id = prof_especialidad_id')
    ...
    public function exportarReporteHistoria(){        
        ...
        $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('J5', 'MOTIVO')
                ->setCellValue('K5', 'PESO INICIAL')
                ->setCellValue('L5', 'PESO FINAL')
                ->setCellValue('M5', 'TALLA INICIAL')
                ->setCellValue('N5', 'TEMPERATURA');

        foreach($rsReporteHistoria  as $value) {
            
            $this->db->where('hd.hid_his_id',$value->his_id);
            $rsdetalles = $this->db->from('historia_detalles hd')
                                  ->join('productos pr','pr.prod_id = hd.hid_producto_id')
                                  ->get()
                                  ->result();
            $this->db->where('ho.hio_his_id',$value->his_id);
            $rsdetallesOtros = $this->db->from('historia_otros ho')
                                 // ->join('productos pr','pr.prod_id = ho.hio_producto_id')
                                  ->get()
                                  ->result();
                // if($value->his_id==30) {
                //   var_dump($rsdetallesOtros);exit();                                          
                  //             }
         ...
         $spreadsheet->getActiveSheet()
                       ...
                       // ->setCellValue('J'.$i, $value->his_proxima_cita)
                        ->setCellValue('J'.$i, $value->motivo)
                        ->setCellValue('K'.$i, $value->peso_ini) 
                        ->setCellValue('L'.$i, $value->peso_fin)
                        ->setCellValue('M'.$i, $value->talla)
                        ->setCellValue('N'.$i, $value->temperatura)  
                        ->setCellValue('O'.$i, $value->his_id);
                        // tratamiento                                          
                ...
                $i++;                
                    ...
                    $spreadsheet->getActiveSheet()
                           ->setCellValue('D'.$i,'TRATAMIENTO :')
                           ->setCellValue('E'.$i,'DESCRIPCION')
                           ->setCellValue('F'.$i,'CANTIDAD')
                           ->setCellValue('G'.$i,'DOSIFICACION');

                    $i++;                           

                       foreach ($rsdetalles as $valor) {
                    
                        $spreadsheet->getActiveSheet()
                           
                           ->setCellValue('E'.$i,$valor->hid_descripcion)
                           ->setCellValue('F'.$i,$valor->hid_cantidad)
                           ->setCellValue('G'.$i,$valor->hid_dosificacion);
                          $i++;
                        
                        }
                      $i++;
                        $spreadsheet->getActiveSheet()
                           ->setCellValue('D'.$i,'OTROS EXAMENES :')
                           ->setCellValue('E'.$i,'DESCRIPCION')
                           ->setCellValue('F'.$i,'CANTIDAD')
                           ->setCellValue('G'.$i,'OBSERVACIONES');
                     $i++;

                     foreach ($rsdetallesOtros as $val) {
                    
                        $spreadsheet->getActiveSheet()
                           
                           ->setCellValue('E'.$i,$val->hio_descripcion)
                           ->setCellValue('F'.$i,$val->hio_cantidad)
                           ->setCellValue('G'.$i,$val->hio_observacion);    
                          $i++;
                        
                        }
                     $i++;
                     $spreadsheet->getActiveSheet()
                           ->setCellValue('D'.$i,'EFERMEDAD Y DIAGNOSTICO :')
                           ->setCellValue('E'.$i,'CODIGO CIE - TIPO ENFERMEDAD')
                           ->setCellValue('F'.$i,'ENFERMEDAD ACTUAL')
                           ->setCellValue('G'.$i,'DIAGNOSTICO')
                           ->setCellValue('H'.$i,'EXAMEN FISICO')
                           ->setCellValue('I'.$i,'PROXIMA CITA')
                           ->setCellValue('J'.$i,'OTROS EXAMENES Y OBSERVACIONES');
                       $i++;
                        $spreadsheet->getActiveSheet()
                           ->setCellValue('E'.$i,$value->codigo_cie)
                           ->setCellValue('F'.$i,$value->enfermedad_actual)
                           ->setCellValue('G'.$i,$value->diagnostico)
                           ->setCellValue('H'.$i,$value->tratamiento)
                           ->setCellValue('I'.$i,$value->his_proxima_cita)
                           ->setCellValue('J'.$i,$value->recomendacion);
                      $i++;
