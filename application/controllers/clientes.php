...
class Clientes extends CI_Controller {
    ...
    public function index($pagina = FALSE){
        ...
        $data['tipo_cliente_rubros'] = $this->tipo_cliente_rubros_model->select();
        $data['zonas'] = $this->zonas_model->select();
        ...
        $data['rubro_select'] = $this->input->post('rubro');
        $data['zona_select']  = $this->input->post('zona');
        $data['placa_select'] = $this->input->post('placa');        
        ...
        $rubro = '';
        if (($this->input->post('rubro') != '') && ($this->input->post('rubro') != 'todos')) {
            $rubro = $this->input->post('rubro');
        }
        $zona = '';
        if (($this->input->post('zona') != '') && ($this->input->post('zona') != 'todos')) {
            $zona = $this->input->post('zona');
        }
        $placa = '';
        if (($this->input->post('placa') != '') && ($this->input->post('placa') != 'todos')) {
        ...
        if($pagina){
         ...
         $data['clientes'] = $this->clientes_model->select('', $estados_cliente, $cliente_id, '',$tipo_cliente, $empleado_id, $estados_contrato,$rubro,$zona,$placa,$limite,$inicio);
        
        
         $config['base_url']    = base_url().'index.php/clientes/index/';
         $config['total_rows']  = count($this->clientes_model->select('', $estados_cliente, $cliente_id, '',$tipo_cliente, $tipo_contrato, $estados_contrato,$rubro,$zona,$placa));
    ...
    public function guardarSubidaClientes() {
        ...
        foreach($sheet->getRowIterator(2) as $row)
        {
            ...
            $cliente['domicilio2'] = $sheet->getCellByColumnAndRow('6', $row->getRowIndex())->getValue();
            $cliente['email'] = $sheet->getCellByColumnAndRow('7', $row->getRowIndex())->getValue();
            $cliente['email2'] = $sheet->getCellByColumnAndRow('8', $row->getRowIndex())->getValue();
            $cliente['email3'] = $sheet->getCellByColumnAndRow('9', $row->getRowIndex())->getValue();
            $cliente['pagina_web'] = $sheet->getCellByColumnAndRow('10', $row->getRowIndex())->getValue();
            $cliente['telefono_fijo_1'] = $sheet->getCellByColumnAndRow('11', $row->getRowIndex())->getValue();
            $cliente['telefono_movil_1'] = $sheet->getCellByColumnAndRow('12', $row->getRowIndex())->getValue();
            $cliente['placa'] = $sheet->getCellByColumnAndRow('13', $row->getRowIndex())->getValue();
            //buscamos el tipo de rubro por descripcion del rubro cliente
            $tipo_rubro=$sheet->getCellByColumnAndRow('14', $row->getRowIndex())->getValue();
            $tiporubroid=$this->db->from("tipo_cliente_rubros")
                                  ->where('tcr_nombre',$tipo_rubro)
                                  ->get()
                                  ->row();
            $cliente['tipo_cliente_rubro_id'] = $tiporubroid->tcr_id;
             $cliente['descuento'] = $sheet->getCellByColumnAndRow('15', $row->getRowIndex())->getValue();
             $cliente['linea_de_credito'] = $sheet->getCellByColumnAndRow('16', $row->getRowIndex())->getValue();

              //buscamos la zona
            $zona = $sheet->getCellByColumnAndRow('17', $row->getRowIndex())->getValue();
            $zonaId = $this->db->from("zonas")
                            ->where("zon_nombre",$zona)
                            ->get()
                            ->row();
            $cliente['zona_id'] = $zonaId->zon_id;
            $cliente['maps'] = $sheet->getCellByColumnAndRow('18', $row->getRowIndex())->getValue();
            $cliente['puntos'] = $sheet->getCellByColumnAndRow('19', $row->getRowIndex())->getValue();
            $cliente['bonus'] = $sheet->getCellByColumnAndRow('20', $row->getRowIndex())->getValue();    
    ...
    public function ExportarExcel($id='',$estado='',$tipo='') {
        ...
        $query = $this->db->get();
        $result = $query->result();
        //$this->db->from("tipo_cliente_rubros");
        */
        $rsCliente = $this->db->from("clientes cl")
                                ->join("tipo_cliente_rubros tcr", "cl.tipo_cliente_rubro_id=tcr.tcr_id","left")
                                ->join("zonas zn", "cl.zona_id=zn.zon_id", "left")
                                ->get()
                                ->result();
        ...
        $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('F1', 'DOMICILIO2')
                ->setCellValue('G1', 'EMAIL')
                ->setCellValue('H1', 'EMAIL2')
                ->setCellValue('I1', 'EMAIL3')
                ->setCellValue('J1', 'PAGINA_WEB')
                ->setCellValue('K1', 'TELEFONO_FIJO_1')
                ->setCellValue('L1', 'TELEFONO_MOVIL_1')
                ->setCellValue('M1', 'PLACA')
                ->setCellValue('N1', 'RUBRO')
                ->setCellValue('O1', 'DESCUENTO')
                ->setCellValue('P1', 'LINEA_DE_CREDITO')
                ->setCellValue('Q1', 'ZONA')
                ->setCellValue('R1', 'UBICACION')
                ->setCellValue('S1', 'PUNTOS')
                ->setCellValue('T1', 'BONUS');
                
            
               // ->setCellValue('E1', 'EMPRESA');

        $spreadsheet->getActiveSheet()->setTitle('clientes');
        foreach ($rsCliente as $value) {
            ...
            $spreadsheet->getActiveSheet()
                        ...
                        ->setCellValue('F'.$i, $value->domicilio2)
                        ->setCellValue('G'.$i, $value->email)
                        ->setCellValue('H'.$i, $value->email2)
                        ->setCellValue('I'.$i, $value->email3)
                        ->setCellValue('J'.$i, $value->pagina_web)
                        ->setCellValue('K'.$i, $value->telefono_fijo_1)
                        ->setCellValue('L'.$i, $value->telefono_movil_1)
                        ->setCellValue('M'.$i, $value->placa)
                        ->setCellValue('N'.$i, $value->tcr_nombre)
                        ->setCellValue('O'.$i, $value->descuento)
                        ->setCellValue('P'.$i, $value->linea_de_credito)
                        ->setCellValue('Q'.$i, $value->zon_nombre)
                        ->setCellValue('R'.$i, $value->maps)
                        ->setCellValue('S'.$i, $value->puntos)
    ...
    public function grabar() {
        ...
        $data = array(
                ...
                'domicilio2'   => trim($this->input->post('domicilio2')),
                ...
                'tipo_cliente_rubro_id' => $this->input->post('rubro'),
    ...
    public function grabar_para_comprobante() {
        ...
        $data = array(
            ...
            'domicilio2' => strtoupper(trim($this->input->post('domicilio2'))),
            'email'   => trim($this->input->post('email')),
            'zona_id' => $this->input->post('zona'),
            'maps' => $this->input->post('maps'),
            'tipo_cliente_rubro_id' => $this->input->post('rubro'),
            'telefono_movil_1' => $this->input->post('telefono_movil_1'),
            'placa' => $this->input->post('placa'),
            'tipo_cliente_rubro_id' => $this->input->post('tipo_cliente_rubro_id'),
            'empresa_id' => 1,
            'activo' => 'activo',
            'empleado_id_insert' => $this->session->userdata('empleado_id'),
            'tipo_cliente_id' => $tipo_cliente[0],
    ...
    public function modificar_g(){        
        ...
        if($_FILES['foto']['name'] != '')
                $data = array_merge($data,array('foto' => $_FILES['foto']['name']));
