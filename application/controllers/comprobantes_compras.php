...
class Comprobantes_compras extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('items_compras_model');
    ...
    public function dashboard() {
        $inicio = $_POST['inicio'];
        $final = $_POST['final'];
        $res= $this->comprobantes_compras_model->getdash($inicio,$final);
    ...
    public function documentos(){        
        ...
        $dataComprobante = $this->comprobantes_compras_model->selectVersion2('', $data['serie_select'], $data['numero_select'], $data['fecha_de_emision_inicio'], $data['fecha_de_emision_final'], $data['cliente_select_id'], $data['tipo_documento_id'], '', '', $inicio, $limite, $data['empresa_id']);
        $data['comprobantes'] = $this->comprobantes_compras_model->formatVoucher($dataComprobante);
        ...
        $config['total_rows'] = $this->comprobantes_compras_model->selectCountVersion2('', $data['serie_select'], $data['numero_select'], $data['fecha_de_emision_inicio'], $data['fecha_de_emision_final'], $data['cliente_select_id'], $data['tipo_documento_id'], '', '', '', '', $data['empresa_id']);
    ...
    public function getMainList() {
        $rsDatos = $this->comprobantes_compras_model->getMainList();
    ...
    public function index() {
        ...
        $dataComprobante = $this->comprobantes_compras_model->select('', $serie, $numero, $fecha_de_emision, $fecha_de_vencimiento, $cliente_id, $tipo_documento_id, '', '', $inicio, $limite, $empresa_id,$numero_pedido,$numero_guia,$orden_compra);
        ...
        $this->accesos_model->menuGeneral();
        $this->load->view('comprobantes_compras/index', $data);
        $this->load->view('templates/footer');
    }

    //ALEXANDER FERNANDEZ 05-12-2020
    public function modal_pagoMonto(){
        $data['tipo_pagos'] =  $this->tipo_pagos_model->select();
        echo $this->load->view('comprobantes_compras/modal_pagoMonto',$data);
    }

    public function agregarAnticipoUi()
    {
        $anticipos = $this->comprobantes_compras_model->listaAnticiposClientes();
        $data = [
                    "anticipos" => $anticipos
                ];
    ...
    {   $error = false;
        ...
        $result = $this->comprobantes_compras_model->guardarAnticipo();       
    }
    ...
    public function indexo($pagina = '') {
        ...
        $data['comprobantes'] = $this->comprobantes_compras_model->select('', '', '', '', '', '', '', '', '', $inicio, $limite, '');

        $config['base_url'] = base_url() . '/index.php/comprobantes/indexo/';
        $config['total_rows'] = count($this->comprobantes_compras_model->select('', '', '', '', '', '', '', '', '', FALSE, FALSE, ''));
        $config['per_page'] = $limite;
        $this->pagination->initialize($config);
    ...
    public function comprobante() {
        ...
        $dataComprobante = $this->comprobantes_compras_model->select('', $serie, $numero, $fecha_de_emision, $fecha_de_vencimiento, $cliente_id, $tipo_documento_id, '', '', $inicio, $limite, $empresa_id);
        $data['comprobantes'] = $this->comprobantes_compras_model->formatVoucher($dataComprobante);
        ...
        $config['total_rows'] = count($this->comprobantes_compras_model->select('', $serie, $numero, $fecha_de_emision, $fecha_de_vencimiento, $cliente_id, $tipo_documento_id, '', '', FALSE, FALSE, $empresa_id));
        $config['per_page'] = $limite;
    ...
    public function index_asesor($pagina = FALSE) {
        ...
        $data['comprobantes'] = $this->comprobantes_compras_model->select('', $serie, $numero, $fecha_de_emision, $fecha_de_vencimiento, $cliente_id, $tipo_documento_id, '', '', $inicio, $limite, $empresa_id);
        //var_dump($data['comprobantes']);exit;
        //PAGINACION        
        $this->load->library('pagination');
    ...
    public function nuevo($factura_id = '', $valida = 0) {
        ...
        $data['comp_adjuntos'] = $this->comprobantes_compras_model->select('', '', '', '', '', '', '', '0');
        ...
        $this->accesos_model->menuGeneral();
        $this->load->view('comprobantes_compras/nuevo', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function modificar() {                
        ...
        $data['comprobante'] = $this->comprobantes_compras_model->select($this->uri->segment(3));  
       ...
       // print_r($data['comprobante']);exit();
        ...
        $data['items'] = $this->items_compras_model->select('', $this->uri->segment(3));
       ...
       /* cargar la configuracion */
        ...
        $this->accesos_model->menuGeneral();
        $this->load->view('comprobantes_compras/modificar', $data);
        $this->load->view('templates/footer');
    }

    public function eliminar($comprobante_id) {
        $this->comprobantes_compras_model->eliminar($comprobante_id);
        redirect(base_url() . "index.php/comprobantes_compras/");
    }
    ...
    public function detalle() {
        ...
        $data['comprobante'] = $this->comprobantes_compras_model->select($comprobante_id);
        $data['items'] = $this->items_compras_model->select('', $comprobante_id);
        ...
        switch ($tipo_documento_id) {
            ...
            case 7:
                ...
                $data['comp_adjunto'] = $this->comprobantes_compras_model->select($data['comprobante']['com_adjunto_id']);
                break;
            case 8:
    ...
    public function pdfGeneraComprobante($comprobante_id = '', $vista = '') {
        $comprobante = $this->comprobantes_compras_model->select($comprobante_id);
        ...
        if ($comprobante['tipo_documento_id'] == 7) {
            ...
            $cadjunto = $this->comprobantes_compras_model->select($comprobante['com_adjunto_id']);
        }
    ...
    public function pdfGeneraComprobanteOffLine($comprobante_id = '', $vista = '') {
        ...
        $rsEmpresa = $query->row();
        /*obtenemos todos los datos del documento*/
        $this->db->from("comprobantes_compras as com")
                 ->join("tipo_documentos as tdoc", "com.tipo_documento_id=tdoc.id")
        ...
        $rsComprobante = $query->row();
        /*obtenemos el detalle del documento*/
        $this->db->select('i.*,p.*,a.*,m.medida_codigo_unidad')
                 ->from("items_compras as i")
                 ->join("productos as p","p.prod_id=i.producto_id",'LEFT')
                 ->join("almacenes as a","a.alm_id =p.prod_almacen_id",'LEFT')
                 ->join("medida as m","m.medida_id=i.unidad_id")
                 ->where("i.comprobante_id", $comprobante_id);
        $query = $this->db->get();
        $rsDetalle = $query->result();
        //var_dump($rsDetalle);exit;

        //PAGO MONTO ALEXANDER FERNANDEZ 14-10-2020
        $rsPagoMonto =  $this->db->from('comprobante_compras_pagos cmp')
        ...
        $rsComprobante->fecha_de_vencimiento = ($rsComprobante->fecha_de_vencimiento!='')?(new DateTime($rsComprobante->fecha_de_vencimiento))->format("d/m/Y"):'';
        /*documento relacionado*/
        $rsRelacionado = $this->db->from("comprobantes_compras")
                                  ->where("id", $rsComprobante->com_adjunto_id)
                                  ->get()
                                  ->row();
       ...
       // $certificado = $this->ObtenerCertificado($rsEmpresa->ruc,$rsComprobante->codigo,$rsComprobante->serie,$rsComprobante->numero);    
        ...
        $html = $this->load->view("templates/invoice_compras.php",$data,true);
    ...
    public function jsonComprobante($factura_id = '') {
        $comprobante = array();
        $comprobante = $this->comprobantes_compras_model->jsonComprobante($factura_id);
    ...
    public function popoverSunat($estado = '', $cliente_id = '') {
        /* ************************************************* */
        $comprobante = $this->comprobantes_compras_model->select($_GET['comprobanteId']);
        $items = $this->items_compras_model->select('', $_GET['comprobanteId']);
    ...
    public function rptaSunat() {
        $query = $this->comprobantes_compras_model->selecRptaSunat(1, 1);
        //var_dump($query);
        $numrow = count($query);
        $nuevoEstado = array();
        if ($numrow > 0) {
            foreach ($query as $comprobante) {                
                ...
                if (!empty($obj)) {
                    ...
                    if ($codSunat == 0) {
                        $this->comprobantes_compras_model->modificar(array('estado_sunat' => $codSunat), $comprobante['comprobante_id']);
                    }
                }
            }
        }
    ...
    public function rptaSunatAnulado() {
        ...
        $estado_anulado = 1; 
        $query = $this->comprobantes_compras_model->selecRptaSunatAnulaciones($estado_anulado);
        $numrow = count($query);
        $nuevoEstado = array();
            ...
            foreach ($query as $comprobante) {
                ...
                if (!empty($obj)) {
                    ...
                    if ($codSunat == 0) {
                        ...
                        $this->comprobantes_compras_model->modificar(array('anulado' => 2), $comprobante['comprobante_id']);
                    }
                }
            }
        }
...
//        echo $obj['contenido'];
    ...
    public function guardar_comprobante() {
        ...
        $serie = (($_POST['tipo_documento'] == 1) || 
                ...
                ($_POST['tipo_documento'] == 8) || 
                ($_POST['tipo_documento'] == 10)|| 
                ($_POST['tipo_documento'] == 13) ) ? strtoupper($_POST['serie']): $_POST['serie_antiguo'];        
        ...
        $comprobante_id = $this->comprobantes_compras_model->insertar($comprobante);
        ...
        foreach ($idproduc as $item) {
            ...
            $this->db->insert("items_compras", $dataDetalleInsert);

            if($result->prod_tipo==1){
                $this->db->set('prod_precio_compra',$importe[$i]);
                $this->db->where('prod_id',$idproduc[$i]);
                $this->db->update('productos');
                    ...
                    if($_POST['tipo_documento'] != 7 and $_POST['tipo_documento'] != 9){
                         ...
                         $this->productos_model->ingresarStock($idproduc[$i] ,$cantidad[$i], $concepto, $comprobante_id, $serie, $numero, $result->prod_almacen_id, 1);
                    } else{
                         $this->db->where('id',$this->input->post('tipo_ncredito'));
                         $concepto = $this->db->get('tipo_ncreditos')->row();
                         $concepto = $concepto->tipo_ncredito;
                         
                         $this->productos_model->salidaStock($idproduc[$i] ,$cantidad[$i], $concepto, $comprobante_id, $serie, $numero, $result->prod_almacen_id, 1); 
                    }                                        
            } 
            $i++;
        }     

        //INSERTAR COMPROBANTES PAGOS 05-12-2020
        ...
        foreach ($tipo_pagoMonto as $value) {
            ...
            $this->db->insert('comprobante_compras_pagos',$pagoMonto);
            $i++;               
        }
    ...
    public function updateEstadoComprobante($comprobante_id = '', $operacion_cancelada = '') {
        ...
        $this->comprobantes_compras_model->modificar($array, $comprobante_id);
    ...
    public function modificar_comprobante() {  
        ...
        $this->comprobantes_compras_model->modificar($comprobante, $comprobante_id);
        ...
        $rsDetalles = $this->db->from("items_compras")
                               ->where("comprobante_id", $comprobante_id)
                               ->get()
                               ->result();
        ...
        foreach ($idproduc as $item) {
           ...
           // $dataDetalleInsert["descripcion"] = $this->quitarSaltoDeLinea($descripcion[$i]);
            ...
            $this->db->where('comprobante_id',$comprobante_id);
            $this->db->where('producto_id',$idproduc[$i]);
            $dato_producto = $this->db->get('items_compras')->row();
            if($result->prod_tipo==1){
    ...
    public function txt($envio = 0, $comprobante_id = '') {
        ...
        if ($envio == 0) {
            ...
            $this->comprobantes_compras_model->modificar(array('enviado_sunat' => 1), $comprobante_id);
            ...
            if ($comprobante['tipo_documento_id'] == 3) {
                ...
                $this->comprobantes_compras_model->modificar(array('fecha_de_baja' => $fecha2, 'anulado' => 1), $comprobante_id);
            } else {
            ...
            ////// STOCK 
                ...
                $this->db->from('items_compras as i');
                $this->db->join('comprobantes_compras as c','c.id=i.comprobante_id');
        ...
        redirect(base_url() . "index.php/comprobantes_compras/index/" . $comprobante['empresa_id']);
    }
    ...
    public function comunicacionBaja() {
        ...
        $data['comprobante'] = $this->comprobantes_compras_model->select('', '', '', '', '', '', '', '', 1);
        $this->accesos_model->menuGeneral();
    ...
    public function selectUltimoReg() {        
        $row = $this->comprobantes_compras_model->selecMaximoNumero2($this->uri->segment(3), $this->uri->segment(4), $this->uri->segment(5));
...
//            '',
    ...
    public function _exportarExcel() {   
        ...
        $comprobantes_items = $this->comprobantes_compras_model->selectCustomizadoDetalle(3, $campos);
        $comprobantes = $this->comprobantes_compras_model->FormatedSelectCustomizadoDetalle($comprobantes_items);
    ...
    public function exportarExcel()
    {
        ...
        //$this->db->where('eliminado', 0);   
        //$this->db->group_by('id');
        $this->db->from("comprobantes_compras");
    ...
    public function exportarExcel_rd()
    {
       ...
       // $resultComprobantes = $query->result();
        ...
        $result = $this->db->select("cc.*,ic.*,td.*,pr.*,mo.*,tp.*, cc.id as id_compra, emp.*, DATE_FORMAT(fecha_de_emision, '%d/%m/%Y') fecha_de_emision  from comprobantes_compras cc",false)
                           ->join("items_compras ic","cc.id=ic.comprobante_id")
                           ->join("tipo_documentos td","td.id=cc.tipo_documento_id")
                           ->join("proveedores pr","pr.prov_id=cc.cliente_id")
                           ->join("monedas mo","mo.id=cc.moneda_id")  
                           ->join("tipo_pagos tp","tp.id = cc.tipo_pago_id")
                           ->join("empleados emp","emp.id = cc.empleado_insert")    
                           ->get()
                           ->result();
        ...
        // metodo para dar ancho a la libreria
        ...
        $objPHPExcel->setActiveSheetIndex(0);
        $objPHPExcel->getActiveSheet()->setCellValue('A1', "N° COMPRA"); 
        $objPHPExcel->getActiveSheet()->setCellValue('B1', "SERIE DOC.");
        $objPHPExcel->getActiveSheet()->setCellValue('C1', "NUMERO DOC.");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('H1', "TIPO DOC.");
        $objPHPExcel->getActiveSheet()->setCellValue('I1', "EMPLEADO");
        $objPHPExcel->getActiveSheet()->setCellValue('J1', "PROVEEDOR"); 
        $objPHPExcel->getActiveSheet()->setCellValue('K1', "FECHA EMISION");        
        $objPHPExcel->getActiveSheet()->setCellValue('L1', "MONEDA");
        ...
        foreach($result as $value)
        {
            ...
            //$importeLetras .= $importeLetras.' con '.$importeAPagar[1].'/100 '.$rsMoneda->moneda;
            $objPHPExcel->getActiveSheet()
                        ->setCellValue('A' . ($i), $value->id_compra)
                        ->setCellValue('B' . ($i), $value->serie)
                        ->setCellValue('C' . ($i), $value->numero)                     
                        ->setCellValue('D' . ($i), $value->descripcion)
                        ->setCellValue('E' . ($i), $value->cantidad)
                        ->setCellValue('F' . ($i), $value->importe)
                        ->setCellValue('G' . ($i), $value->total)
                        ->setCellValue('H' . ($i), $value->tipo_documento)
                        ->setCellValue('I' . ($i), $value->nombre)
                        ->setCellValue('J' . ($i), $value->prov_razon_social)
                        ->setCellValue('K' . ($i), $value->fecha_de_emision)
                        ->setCellValue('L' . ($i), $value->moneda);
    ...
    public function cargarComprobantesCliente() {
        ...
        $comprobantes = $this->comprobantes_compras_model->select('', '', '', '', '', $cliente_id, '', $com_adjuntos, $anulado = '', FALSE, FALSE, $empresa_id);
    ...
    public function comprobantesNotasCredito() {
        ...
        $comprobantes = $this->comprobantes_compras_model->select('', '', '', '', '', $cliente_id, $tipo_documento_id, '', $anulado = '', FALSE, FALSE, $empresa_id);
