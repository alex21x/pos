...

{
    ...
    public function exportarExcel(){
        ...
        public function descargarPdf($idPaciente) {
          
        require_once (APPPATH .'libraries/Numletras.php');
        
        $rspaciente = $this->db->from("pacientes as pc")
                                ->join("tipo_pacientes tp","pc.pac_tipo_id=tp.tipo_pac_id",left)     
                                ->join("sexos se","pc.sexo=se.id")                     
                                ->where("pc.id", $idPaciente)
                                ->get()
                                ->row();             
        /*formateamos fecha*/
        $rspaciente->fecha_nacimiento = (new DateTime($rspaciente->fecha_nacimiento))->format('d/m/Y');                                     
        $rsempresa = $this->db->from('empresas')
                             ->where('id',1)
                              ->get()
                              ->row();

        $rsmedico = $this->db->from('profesionales')
                              ->where('prof_id',$rspaciente->medico_id)
                              ->get()
                              ->row();
        
        $rsCliente =  $this->db->from("clientes")
                              ->where("id", $rspaciente->cliente_id)
                              ->get()
                              ->row();       
        ...
        $html = $this->load->view("templates/paciente.php",$data,true);       
