...
class Reportes_model extends CI_Model {
    ...
    public function reporteUtilidades_ct(){
        ...
        $rsReporteComprobantes = $this->db->select("pro.prod_id prod_id,pro.prod_codigo prod_codigo,pro.prod_nombre prod_nombre,SUM(itm.cantidad) cantidad,SUM(itm.subtotal) total_venta,cat.cat_nombre,lin.lin_nombre,mar.mar_nombre,med.medida_nombre,alm.alm_nombre")
                                          ...
                                          ->join('almacenes alm','alm.alm_id =  com.venta_almacen_id')
                                          ->where('com.anulado',0)
    ...
    public function reporteUtilidades_nv(){
        ...
        $rsReporteNotaVenta = $this->db->select("pro.prod_id prod_id,pro.prod_codigo prod_codigo,pro.prod_nombre prod_nombre,SUM(nde.notapd_cantidad) cantidad,SUM(nde.notapd_subtotal) total_venta,cat.cat_nombre,lin.lin_nombre,mar.mar_nombre,med.medida_nombre,alm.alm_nombre")
                                          ...
                                          ->join('lineas lin','lin.lin_id = pro.prod_linea_id','left')
                                          ->join('marcas mar','mar.mar_id = pro.prod_marca_id','left')
                                          ->join('almacenes alm','alm.alm_id =  npe.notap_almacen')
    ...
    public function reporte_liquidacionReparto_ct(){
        ...
        $rsReporteComprobantes = $this->db->select("com.id comprobante_id,com.serie serie,concat_ws('-', com.serie, com.numero) numser,com.total_igv total_igv,com.total_a_pagar total_a_pagar,(SELECT SUM(totalCosto) FROM items ite WHERE ite.comprobante_id = com.id ) total_costo,
            DATE_FORMAT(com.fecha_de_emision, '%d-%m-%Y') fecha_de_emision,cli.razon_social cliente_razon_social,CONCAT(epl.nombre,' ', epl.apellido_paterno) vendedor,tdc.tipo_documento tipo_documento,tpg.tipo_pago tipo_pago,alm.alm_nombre",FALSE)
                                          ->from('comprobantes com')
    ...
    public function reporte_impresionLotes(){      
      ...
      $tipo_documento   = isset($_POST['tipo_documento']) ? $_POST['tipo_documento'] : $_GET['tipo_documento'];
      ...
      $vendedor     = isset($_POST['vendedor']) ? $_POST['vendedor'] : $_GET['vendedor'];
      $busqueda     = isset($_POST['busqueda']) ? $_POST['busqueda'] : $_GET['busqueda'];
      switch ($tipo_comprobante) {
        ...
        if($vendedor != ''){            
            $this->db->where('np.notap_vendedor',$vendedor);
        }           
        if($busqueda != ''){            
            $this->db->where('np.notap_vendedor',ST_ELIMINADO);
        }           
            
           $documento = $this->db->select('np.notap_id documento_id,concat_ws("-", "NP", np.notap_correlativo) nDocumento, np.notap_fecha fecha_de_emision,np.notap_total total_a_pagar,cli.razon_social razon_social_cliente,CONCAT(emp.nombre," ",emp.apellido_paterno) as vendedor,alm.alm_nombre',FALSE)
                                          ->from('nota_pedido np')
        ...
        case ST_COMPROBANTE:
          ...
          if($tipo_documento != ''){            
            $this->db->where('com.tipo_documento_id',$tipo_documento);
          }          
          if($numero_inicial != ''){            
          ...
          if($busqueda != ''){            
            $this->db->where('com.empleado_select',ST_ELIMINADO);
          }     

            $documento = $this->db->select('com.id documento_id, concat_ws("-", com.serie, com.numero) nDocumento, com.fecha_de_emision fecha_de_emision,com.total_a_pagar,cli.razon_social razon_social_cliente,CONCAT(emp.nombre," ",emp.apellido_paterno) as vendedor,alm.alm_nombre',FALSE)
                                          ->from('comprobantes com')
                                          ->join('clientes cli','com.cliente_id = cli.id')
    ...
    //REPORTE POR LOTES
    public function reporte_impresionLotes_01(){
      ...
      switch ($tipo_comprobante) {
        ...
        if($vendedor != ''){            
         ...
         if($busqueda != ''){            
           ...
           $documento = $this->db->select('np.notap_id documento_id,"NP" nserie, np.notap_correlativo nDocumento, np.notap_fecha fecha_de_emision,np.notap_total total_a_pagar,cli.razon_social razon_social_cliente,cli.ruc cliente_ruc,cli.domicilio1 cliente_domicilio,CONCAT(emp.nombre," ",emp.apellido_paterno) as vendedor,alm.alm_nombre,"1" tipo_comprobante,mon.moneda',FALSE)
                                          ->from('nota_pedido np')
                                          ->join('clientes cli','np.notap_cliente_id = cli.id')
                                          ->join('empleados emp','np.notap_vendedor = emp.id')
                                          ->join('almacenes alm','np.notap_almacen = alm.alm_id')
            ...
            $ticketHeight_total = 0;
            foreach ($documento as $key => $rs) {
                            $rsDetalles = $this->db->select("npd.notapd_cantidad cantidad,npd.notapd_descripcion descripcion,npd.notapd_precio_unitario importe,npd.notapd_total total")
                                                   ->from("nota_pedido_detalle npd")
                                                   ->where("npd.notapd_notap_id", $rs->documento_id)
                                                   ->get()
                                                   ->result();
                            $rsPagos = $this->db->from('nota_pagos cmp')
                                                ->join('tipo_pagos tpg','cmp.tipo_pago_id =  tpg.id')
                                                ->where('cmp.nota_id', $rs->documento_id)
                                                ->get()
                                                ->result();
                $countItems = count($rsDetalles);
                $ticketHeight =  $countItems*35;                
                $ticketHeight_total =  $ticketHeight_total + $ticketHeight;


                $rs->detalles = $rsDetalles;
                $rs->pagoMonto = $rsPagos;

                //Convetimos el total en texto
                $num = new Numletras();
                $totalVenta = explode(".",$rs->total_a_pagar);
                $totalLetras = $num->num2letras($totalVenta[0]);
                $totalLetras = $totalLetras.' con '.$totalVenta[1].'/100 '.$rs->moneda;
                $rs->total_letras = $totalLetras;
            }
            //$ticketHeight_total = ($ticketHeight_total > 440) ? $ticketHeight_total : 440;
            //$documento->ticketHeightTotal = $ticketHeight_total;
            break;
        ...
        case ST_COMPROBANTE:
          ...
          if($busqueda != ''){            
            ...
            $documento = $this->db->select('com.id documento_id, com.serie nserie , com.numero nDocumento, com.fecha_de_emision fecha_de_emision,com.total_gratuita,com.total_exonerada,com.total_gravada,com.total_inafecta,com.total_igv,com.total_a_pagar,com.total_icbper,com.firma_sunat,cli.razon_social razon_social_cliente,cli.ruc cliente_ruc,cli.domicilio1 cliente_domicilio,CONCAT(emp.nombre," ",emp.apellido_paterno) as vendedor,alm.alm_nombre,"2" tipo_comprobante,mon.moneda,tdc.id tipo_documento_id,tdc.tipo_documento',FALSE)
                                          ->from('comprobantes com')
                                          ->join('clientes cli','com.cliente_id = cli.id')
                                          ->join('empleados emp','com.empleado_select = emp.id')
                                          ->join('almacenes alm','com.venta_almacen_id = alm.alm_id')
             ...
             foreach ($documento as $key => $rs) {
                            $rsDetalles = $this->db->select("ite.cantidad cantidad,ite.descripcion descripcion,ite.importe importe,ite.total total")
                                                   ->from("items ite")
                                                   ->where("ite.comprobante_id", $rs->documento_id)
                                                   ->get()
                                                   ->result();

                            $rsPagoMonto =  $this->db->from('comprobante_pagos cmp')
                                                     ->join('tipo_pagos tpg','cmp.tipo_pago_id =  tpg.id')
                                                     ->where('cmp.comprobante_id', $rs->documento_id)
                                                     ->get()
                                                     ->result();   

                $rs->detalles = $rsDetalles;
                $rs->pagoMonto = $rsPagoMonto;                
