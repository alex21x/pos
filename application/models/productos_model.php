...
class Productos_model extends CI_Model
{
    ...
    public function select($id = '', $activo = '') {
        if ($id != '') {            
            $datos = $this->db->from('productos pr')                            
                              ...
                              ->join("lineas li","pr.prod_linea_id=li.lin_id",'left')
                              ->where('prod_id',$id)
       ...
       $result= $this->db->from("productos")
        ...
        if($_POST['prod_id']!='')
        {
            $dataUpdate = [
                ...
                if($_FILES['prod_imagen']['name'] != '')
                  $dataUpdate = array_merge($dataUpdate,array('prod_imagen' => $_FILES['prod_imagen']['name']));
     ...
     *-----------------------------------------------------------   
     * SE AUMENTÓ P1,P2,P3,P4,P5 / ALEXADER FERNANDEZ / 15-04-202 */
   ...
   public function selectAutocompleteprodSCa($buscar){ 
    ...
    public function ingresarStock($prod_id ,$cantidad, $concepto = 'INGRESO',$comprobante_id = '',$serie = '',$numero = '',$almacen_id = '',$tipo = ''){
      ...
      $nueva_cantidad = floatval($stock) + floatval($cantidad);
              ...
              $kardex = array(
                   ...
                   'k_vendedor' => $this->session->userdata('empleado_id'),
                   'k_tipo' => $tipo,
    ...
    public function salidaStock($prod_id ,$cantidad, $concepto = 'SALIDA',$comprobante_id = '',$serie = '',$numero = '',$almacen_id = '',$tipo = ''){      
      $almacen_id = ($almacen_id == '') ? $this->session->userdata("almacen_id") : $almacen_id;
      $tipo = ($tipo == '') ? 5 : $tipo;
      $stock = $this->getStockProductos($prod_id,$almacen_id);
            $nueva_cantidad = floatval($stock) - floatval($cantidad);
    ...
    //CODIGO DE BARRA 10-11-2020
    public function selectPrecioCodProducto($prod_codigo_producto = '') {          
        if ($prod_codigo_producto != '') {            
            $datos = $this->db->from('productos pr')                            
                              ->join("categoria cat","pr.prod_categoria_id=cat.cat_id")
                              ->join("medida md","pr.prod_medida_id=md.medida_id")
                              ->where('prod_id',$prod_codigo_producto)
                              ->where('prod_estado',ST_ACTIVO)
                              ->get()
                              ->row();            
          }    
    ...
    public function listaProductosPos(){
      ...
      $where = '(pr.prod_estado = '.ST_ACTIVO.' AND pr.prod_nombre LIKE "%'.$_POST['productoText'].'%") OR (pr.prod_estado='.ST_ACTIVO.' AND pr.prod_codigo LIKE "%'.$_POST['productoText'].'%") OR (pr.prod_estado ='.ST_ACTIVO.' AND pr.prod_codigo_barra LIKE "%'.$_POST['productoText'].'%")';
        ...
        foreach ($rsProductos as $rsProducto) {
            ...
            $stock = $this->getStockProductos($rsProducto->prod_id,$this->session->userdata("almacen_id"));
            $producto_stock = ($stock != '') ? $stock:'0';
