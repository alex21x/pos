...
class Clientes_model extends CI_Model {
    ...
    public function select($id = '', $activo = '', $id_listado = '', $empresa = '',$tipo_cliente = '', $empleado_id = '', $activo_contrato = '',$rubro = '',$zona = '', $placa ='',$limit = FALSE, $start = FALSE){
        $where  = '';
        $limite = '';
        ...
        $where .= ($rubro  != '')  ? " AND cli.`tipo_cliente_rubro_id` = " . $rubro : '';
        $where .= ($zona   != '')  ? " AND cli.`zona_id` = " . $zona : '';
        $where .= ($placa  != '')  ? " AND cli.`placa`= '" . $placa."'" : '';
        if (($limit !== FALSE) && ($start !== FALSE))
            $limite .= " LIMIT ".$start .', '.$limit;
        
        if ($id != '' && $id_listado == '') {
            $rsCliente = $this->db->from('clientes cli')
                                  ->join('tipo_cliente_rubros rub', 'cli.tipo_cliente_rubro_id = rub.tcr_id','LEFT')
                                  ->join('zonas zon ','cli.zona_id = zon.zon_id','LEFT')
                                  ->where('id',$id)
                                  ->get()
                                  ->row_array();            
            return $rsCliente;
        ...
        if (($tipo_contrato != '') || ($activo_contrato != '')){            
            $sql = "SELECT
                ...
                cli.`maps`,    
                epr.`empresa` empresa,
                rub.`tcr_nombre` rubro,
                zon.`zon_nombre` zona
                FROM clientes cli
                JOIN `contratos` con ON cli.`id` = con.`cliente_id`
                JOIN `empresas` epr ON cli.`empresa_id` = epr.`id`
                LEFT JOIN tipo_cliente_rubros rub ON cli.`tipo_cliente_rubro_id` = rub.`tcr_id`
                LEFT JOIN zonas zon ON cli.`zona_id` = zon.`zon_id`
     ...
     *-----------------------------------------------------------   
     * LEFT TIPO_RUBRO_CLIENTES / ALEXADER FERNANDEZ / 15-04-202 */
    ...
    public function selectAutocomplete($buscar, $activo = ''){
        ...
        $sql = "SELECT
                ...
                tcr_precio_text
                FROM clientes cli
                LEFT JOIN tipo_cliente_rubros tcp ON cli.`tipo_cliente_rubro_id` = tcp.`tcr_id`
                WHERE
                (ruc LIKE '%$buscar%'
        ...
        if ($query->row()) {
            foreach ($query->result_array() as $tsArray){
                ...
                $data[] = array(
                    ...
                    "placa"      => $tsArray['placa'],
                    "precio_text"=> $tsArray['tcr_precio_text'],
                    "id" => $tsArray['id']
                );
            }
        }
