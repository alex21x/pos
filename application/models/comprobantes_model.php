    ...
    public function select($id = '',$serie = '',$numero = '',$fecha_de_emision = '',$fecha_de_vencimiento = '',$cliente_id = '',$tipo_documento_id = '',$com_adjuntos = '',$anulado = '',$inicio = FALSE,$limite = FALSE, $empresa_id = '',$numero_pedido='',$numero_guia='',$orden_compra=''){
        if($id != ''){
            $sql = "SELECT com.incluye_igv,com.fecha_de_emision fecha_sunat,com.numero_tarjeta,com.tipo_pago_id as pago_id, "
                    ...
                    . " com.transportista_id transportista_id, "
                    . " com.serie serie, "
                    . " com.numero numero, "
                    ...
                    . " cli.ruc cliente_ruc, "
                    . " cli.telefono_movil_1 cliente_telefono_movil, "
...
$sql = "SELECT COUNT(com.id) as comprobante_id "
    ...
    public function insertar($data) {
        $rs = $this->db->insert('comprobantes', $data);
        if(!$rs){
    ...
    public function getMainList() {
        ...
        {
            $fe = (new DateTime($_POST['fecha_desde']))->format("Y-m-d"); 
            $this->db->where("DATE(comp.fecha_de_emision) >=", $fe);
        }else{
            //$this->db->where("DATE(comp.fecha_de_emision) >=", date('Y-m-d'));
        }
        if($_POST['fecha_hasta']!='')
        {
            $fh = (new DateTime($_POST['fecha_hasta']))->format("Y-m-d");
            $this->db->where("DATE(comp.fecha_de_emision) <=", $fh);
        }else{
            //$this->db->where("DATE(comp.fecha_de_emision) <=", date('Y-m-d'));
        }
        ...
        $select = $this->db->select('emp.ruc as ruc, comp.id as comp_id,cli.ruc cli_ruc,cli.razon_social as cliente,comp.tipo_documento_id, tpd.codigo as codigo,
                                    tpd.tipo_documento as documento,comp.total_gravada as total_gravada,comp.fecha_de_emision as fecha_de_emision,DATE_ADD(comp.fecha_de_emision, INTERVAL 1 DAY) fecha_de_emision_PSE,comp.enlace_del_xml enlace_del_xml,comp.enlace_del_cdr enlace_del_cdr,comp.fecha_de_vencimiento as fecha_de_vencimiento, comp.enviado_sunat as enviado_sunat,comp.estado_sunat,
                                    comp.cliente_id as cliente_id, comp.anulado as anulado,comp.serie as serie,comp.rechazado as rechazado,comp.numero as numero,
                                    comp.total_igv as total_igv,comp.total_a_pagar,CONCAT(e.nombre," ",e.apellido_paterno) as vendedor,m.moneda',false)
                           ->from("comprobantes comp")
                           ->join("clientes cli", "comp.cliente_id = cli.id")
                           ...
                           ->join("monedas m","comp.moneda_id = m.id")
                           ->where("comp.venta_almacen_id", $this->session->userdata("almacen_id"))
                           ->order_by("comp.id", "desc");
                           //->get()
                           //->result();


        //obtniendo total anulados, generados , enviados
        $selecGenerado = clone $select;
        $totalpendientes = count($selecGenerado->where('comp.estado_sunat',1)->get()->result());

        $selecAnulado = clone $select;
        $totalAnulados = count($selecAnulado->where('comp.estado_sunat',5)->get()->result());

        $selecTributario = clone $select;
        $totalenvidados = count($selecTributario->where('comp.estado_sunat',3)->get()->result());                           
        ...
        //CAMBIOS - OBTENER DATOS DE CONFIGURACIÓN ALEXANDER FERNANDEZ //15-02-20121
        $rsConfiguracion = $this->db->from('comprobantes_ventas')
                                    ->get()
                                    ->row();
         
        foreach($rsComprobante as $comprobante)  {

            $comprobante->totalpendientes =  '<button class="btn-pendientes" data-id="'.$totalpendientes.'"><i class="glyphicon glyphicon-pencil"></i></button>';
             $comprobante->totalAnulados  =  '<button class="btn-Anulados" data-id="'.$totalAnulados.'"><i class="glyphicon glyphicon-pencil"></i></button>';
             $comprobante->totalenvidados  ='<button class="btn-enviados" data-id="'.$totalenvidados.'"><i class="glyphicon glyphicon-pencil"></i></button>'; 
            ...
            $comprobante->btn_ticket = '<a class="print_ticket_pdf" idval="'.$comprobante->comp_id.'"  target="_blank"><span class="glyphicon glyphicon-print esunat" aria-hidn="true"></span></a>';
            $comprobante->btn_mail = '<span class="glyphicon glyphicon-remove"></span>';
            $comprobante->btn_popup = '<a href="'.base_url().'index.php/comprobantes/index/'.$comprobante->comp_id.'"><span class="glyphicon glyphicon-file"></a>';

            $comprobante->num_rows = $i;
            $comprobante->cliente = "<label style='color:#34495E;font-size:14px;font-weight:600;'>".$comprobante->cliente."</label>";
            $comprobante->Tipo_doc =$comprobante->documento;
            $comprobante->Monto_bruto =$comprobante->total_gravada;
            $comprobante->num_doc = "<label style='color:#34495E;font-size:14px;font-weight:600;'>".$comprobante->serie."-".$comprobante->numero."</label>";
            ...
            if($comprobante->tipo_documento_id==3){
                $comprobante->Tipo_doc = 'BOL';
            }else if($comprobante->tipo_documento_id==1){
                $comprobante->Tipo_doc = 'FAC';
            }else if($comprobante->tipo_documento_id==7 or $comprobante->tipo_documento_id==9){
                $comprobante->Tipo_doc = 'NC';
            }else if($comprobante->tipo_documento_id==8 or $comprobante->tipo_documento_id==10){
                $comprobante->Tipo_doc = 'ND';
            }

            if ($comprobante->estado_sunat == 1){
                $comprobante->btn_action ='<div class="btn-group">
                      <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background:#37474F;border:0;"> &nbsp;&nbsp;&nbsp;No Generado&nbsp;&nbsp; <span class="caret"></span></button><ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2"><li><a href="'.base_url().'index.php/comprobantes/modificar/'.$comprobante->comp_id.'"> Modificar </a></li>';

                //TOKEN PSE 09-01-2021
                if($this->session->userdata('pse_token') == 'inactivo'){
                    $comprobante->btn_action.= '<li><a class="envSunat" onclick="generate_xml('.$comprobante->comp_id.')"><input type="hidden" value="'.$comprobante->comp_id.'/'.$comprobante->cliente_id.'"> Generar XML </a></li>';
                }else{
                    $comprobante->btn_action.= '<li><a class="envNubefact" onclick="send_xmlPSE('.$comprobante->comp_id.')"><input type="hidden" value="'.$comprobante->comp_id.'/'.$comprobante->cliente_id.'"> Enviar a PSE-SUNAT </a></li>';
                }                
                $comprobante->btn_action.= '</ul></div>';

            }else if ($comprobante->estado_sunat == 2){
                $comprobante->btn_action ='<div class="btn-group">
                      <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="border:0;"> &nbsp;&nbsp;&nbsp;XML Generado&nbsp;&nbsp; <span class="caret"></span></button><ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2"><li><a href="'.base_url().'index.php/comprobantes/modificar/'.$comprobante->comp_id.'"> Modificar </a></li>';
                ...
                //TOKEN PSE 09-01-2021
                if($this->session->userdata('pse_token') == 'inactivo'){
                    $comprobante->btn_action.= '<li><a class="envSunat" onclick="send_xml('.$comprobante->comp_id.')"><input type="hidden" value="'.$comprobante->comp_id.'/'.$comprobante->cliente_id.'"> Enviar a SUNAT </a></li>';
                }else{
                    $comprobante->btn_action.= '<li><a class="envNubefact" onclick="send_xmlPSE('.$comprobante->comp_id.')"><input type="hidden" value="'.$comprobante->comp_id.'/'.$comprobante->cliente_id.'"> Enviar a PSE-SUNAT </a></li>';
                }              
                $comprobante->btn_action.= '</ul></div>';

            }else if ($comprobante->estado_sunat == 3){
                
                      $comprobante->btn_mail = '<a class="enviar_correo" _href="'.base_url().'index.php/comprobantes/Send_Mail/'.$comprobante->comp_id.'"><span class="glyphicon glyphicon-envelope "></span></a>';
        
                      $comprobante->btn_action ='<div class="btn-group">
                      <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background:#28B463;border:0;"> Enviado a Sunat <span class="caret"></span></button>';
                      $comprobante->btn_action.='<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">';

                       //TOKEN PSE 09-01-2021
                      if($this->session->userdata('pse_token') == 'inactivo'){
                        $comprobante->btn_action.='<li><a class="_dow_xml" target="_blank"  href="'.RUTA_API.'index.php/Sunat/dowload_xml/'.$comprobante->ruc.'/'.$comprobante->codigo.'/'.$comprobante->serie.'/'.$comprobante->numero.'"><span class="glyphicon glyphicon-file esunat"></span> XML</a></li>';
                        $comprobante->btn_action.='<li><a class="_dow_cdr" target="_blank" href="'.RUTA_API.'index.php/Sunat/dowload_cdr/'.$comprobante->ruc.'/'.$comprobante->codigo.'/'.$comprobante->serie.'/'.$comprobante->numero.'"><i class="glyphicon glyphicon-list-alt"></i> CDR</a></li>';                        
                      } else{
                        $comprobante->btn_action.='<li><a class="_dow_xml" target="_blank"  href="'.$comprobante->enlace_del_xml.'"><span class="glyphicon glyphicon-file esunat"></span> XML</a></li>';
                        $comprobante->btn_action.='<li><a class="_dow_cdr" target="_blank" href="'.$comprobante->enlace_del_cdr.'"><i class="glyphicon glyphicon-list-alt"></i>CDR</a></li>';
                      }
                    
                      //TOKEN PSE 09-01-2021
                      if($this->session->userdata('pse_token') == 'inactivo'){
                        if($rsConfiguracion->passwordDelete == 1){
                            $comprobante->btn_action.='<li><a onclick="send_anulacionPASSWORD(1,'.$comprobante->comp_id.')" >X Anular </a></li>';
                        } else{
                            $comprobante->btn_action.='<li><a onclick="send_anulacion(1,'.$comprobante->comp_id.')" >X Anular </a></li>';
                        }                    
                      } else {
                        //ANULACIÓN PARA BOLETAS 12-01-2021
                         if($comprobante->tipo_documento_id == 3){
                            if(date('Y-m-d h:i:s') > $comprobante->fecha_de_emision_PSE){
                                if($rsConfiguracion->passwordDelete == 1){
                                    $comprobante->btn_action.='<li><a onclick="send_anulacionPSEPASSWORD(1,'.$comprobante->comp_id.')" >X Anular PSE-SUNAT </a></li>';    
                                }else {
                                    $comprobante->btn_action.='<li><a onclick="send_anulacionPSE(1,'.$comprobante->comp_id.')" >X Anular PSE-SUNAT </a></li>';    
                                }                                
                            }                                                        
                        }else{
                            if($rsConfiguracion->passwordDelete == 1){
                      ...
                      $comprobante->btn_action.='<li><a href="'.base_url().'index.php/comprobantes/nuevo/1/1/'.$comprobante->tipo_documento_id.'/'.$comprobante->comp_id.'" >Nota de Crédito </a></li>';
                      $comprobante->btn_action.='<li><a href="'.base_url().'index.php/comprobantes/nuevo/1/2/'.$comprobante->tipo_documento_id.'/'.$comprobante->comp_id.'" >Nota de Débito</a></li>';
                      $comprobante->btn_action.='</ul></div>'; 

            }else if ($comprobante->estado_sunat == 4){
                $comprobante->btn_action ='<div class="btn-group">
                      <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background:#F39C12;border:0;"> Anulación Pendiente <span class="caret"></span></button><ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">';

                $comprobante->btn_action.= '<li><a class="envSunat" onclick="send_anulacion(1,'.$comprobante->comp_id.')"><input type="hidden" value="'.$comprobante->comp_id.'/'.$comprobante->cliente_id.'"> Consultar estado de anulación </a></li>';
                $comprobante->btn_action.= '</ul></div>';

            }else if ($comprobante->estado_sunat == 5){
                $comprobante->btn_action ='<div class="btn-group">
            ...
            }else if ($comprobante->estado_sunat == 6){
                $comprobante->btn_action ='<div class="btn-group">
                      <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background:#F39C12;border:0;"> Enviado a resumen </button></div>';
            }
    ...
    public function getMainListDetail()
    {
        ...
        $select = $this->db->from("items")
                           ->where("comprobante_id", $_POST['comprobante_id']);                           
   ...
   //////// LE //////////
    ...
    public function selecReporteCaja($fecha = ''){

        $rsComprobante = $this->db->select("sum(total_a_pagar) montoTotal,tipo_pago_id")
                                    ->from('comprobantes')
                                    ->where('fecha_de_emision >',$fecha)
                                    ->where('fecha_de_emision <',date('Y-m-d H:i:s'))  
                                    ->group_by('tipo_pago_id')
                                    ->get()
                                    ->result();

        //var_dump($rsComprobante);
        $array = array();
        foreach ($rsComprobante as $key => $value) {

            if($value->tipo_pago_id == ST_PAGO_EFECTIVO)
                $array['totalContado'] = $value->montoTotal;
            if($value->tipo_pago_id ==ST_PAGO_DEPOSITO)
                $array['totalDeposito'] = $value->montoTotal; 
            if($value->tipo_pago_id == ST_PAGO_CHEQUE)
                $array['totalCheque'] = $value->montoTotal;

            if($value->tipo_pago_id == ST_PAGO_TARJETA)
                $array['totalTarjeta'] = $value->montoTotal;
            if($value->tipo_pago_id ==ST_PAGO_CUPON)
                $array['totalCupon'] = $value->montoTotal;     

               if($value->tipo_pago_id ==ST_PAGO_CREDITO)
                $array['totalCredito'] = $value->montoTotal; 
    ...
    public function select_comprobanteTributario($id = '')
    {
        $rsComprobante = $this->db->from("comprobantes as com")
                           ->join("clientes as cli", "com.cliente_id=cli.id")
                           ->where("com.id", $id)
                           ->get()
                           ->row();
                           //echo count($rsNota);exit;
        //$rsComprobante->com_fecha = (new DateTime($rsNota->notap_fecha))->format('d-m-Y'); 
        //obtenemos los detalles
        $rsDetalles = $this->db->from("items")
                               ->where("comprobante_id", $id)
                               ->get()
                               ->result();
        $rsComprobante->detalles = $rsDetalles;       
