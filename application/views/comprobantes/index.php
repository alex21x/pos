...
<div style="width: 96%;margin: 0 auto; padding: .4rem 0;">
    ...
    $("#grid").kendoGrid({
        ...
        columns: [
                    ...
                    {hidden:'totalAnulados',title:'Nº PEDIDO',width:'60px',template:"#=totalAnulados #"},
                    {hidden:'totalenvidados',title:'Nº PEDIDO',width:'60px',template:"#=totalenvidados #"},
        ...
        dataBound:function(e){            
           ...
           // forzando el boton pendiente
            $(".btn-pendientes").click(function(e){
               var pendientes = $(this).data('id');
            $("#pendientes").html('');       
             $("#pendientes").html('<p style="text-align:center;font-size:16px;background:#337ab7;color:white"><b>TOTAL PENDIENTES: '+pendientes+'</b></p>');
            });
            $('.btn-pendientes').trigger('click');

            // forzando el boton total anulados
            $(".btn-Anulados").click(function(e){
               var anulados = $(this).data('id');
            $("#anulados").html('');       
             $("#anulados").html('<p  style="text-align:center;font-size:16px;background:#FF3F33;color:white"><b>TOTAL ANULADOS : '+anulados+'</b></p>');
            });
            $('.btn-Anulados').trigger('click');

            // forzando el boton enviados
            $(".btn-enviados").click(function(e){
               var enviados = $(this).data('id'); 
             $("#enviados").html('');       
             $("#enviados").html('<p style="text-align:center;font-size:16px;background:#5cb85c;color:white"><b>TOTAL ENVIADOS: '+enviados+'</b></p>');
            });
            $('.btn-enviados').trigger('click');
    ...
    function send_anulacionPSE(opc,comprobante_id){
     ...
     //PSE ANULACIÓN PASSSWORD PSE 26-03-2021
     function send_anulacionPSEPASSWORD(opc,comprobante_id){    
            $.confirm({
                title: 'Anular Comprobante!',
                content: '' +
                '<form action="" class="formName">' +
                ...
                buttons: {
                    formSubmit: {
                        ...
                        action: function () {
                            ...
                            $.getJSON("<?PHP echo base_url(); ?>index.php/comprobantes/getDatosAnulacion_PSEPASSWORD",{opc,comprobante_id,password})
                             .done(function(json){
